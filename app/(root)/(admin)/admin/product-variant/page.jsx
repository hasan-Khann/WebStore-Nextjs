...
import { Plus, Layers, Trash2, Database } from "lucide-react";

export default function ShowVariants() {
  const columns = useMemo(() => DT_VARIANT_CONFIG, []);

  const action = useCallback((row, deleteType, handleDelete) => [
    <EditAction key="edit" href={ADMIN_PRODUCT_VARIANT_UPDATE(row.original._id)}/>,
      ...
      <div className="bg-white/90 dark:bg-zinc-950/90 backdrop-blur-md border-b border-zinc-200 dark:border-zinc-800 sticky top-0 md:top-[72px] z-[40] px-4 md:px-8 py-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <Button variant="outline" size="sm" asChild className="rounded-lg h-9 border-zinc-200 dark:border-zinc-800">
            <Link href={ADMIN_PRODUCT_VARIANT_TRASH} className="flex items-center gap-2">
          ...
          <Button size="sm" asChild className="bg-indigo-600 hover:bg-indigo-700 rounded-lg h-9 shadow-md shadow-indigo-100 dark:shadow-none">
            <Link href={ADMIN_PRODUCT_VARIANT_ADD} className="flex items-center gap-2">
              <Plus size={18} strokeWidth={2.5} />
              <span className="font-bold text-[11px] uppercase tracking-wider">New Variant</span>
            </Link>
          </Button>
        </div>
      </div>
        ...
        <div className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <p className="text-[10px] font-black text-indigo-600 uppercase tracking-[0.2em]">Attribute Manager</p>
            </div>
            <h2 className="text-xl font-bold text-zinc-800 dark:text-zinc-200">SKU Specific Variants</h2>
            <p className="text-xs text-zinc-500 mt-1 max-w-md leading-relaxed">
              Define unique stock levels (SKUs), pricing overrides, and product attributes for specific item variations.
            </p>
          </div>
